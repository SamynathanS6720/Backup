...
public class DriverController {
	...
	public ResponseEntity<DriverModel> updateDriver(@PathVariable Long id, @RequestBody DriverModel driverCoordinate){
		DriverModel Coordinate = driverRepository.findById(id)
				.orElseThrow(() -> new ResourceNotFoundException("Employee not exist with id :" + id));
		
		Coordinate.setFirstName(driverCoordinate.getFirstName());
		Coordinate.setLastName(driverCoordinate.getLastName());
		Coordinate.setEmailId(driverCoordinate.getEmailId());
		Coordinate.setMobileNo(driverCoordinate.getMobileNo());

		DriverModel updatedDriverDetails = driverRepository.save(Coordinate);
		return ResponseEntity.ok(updatedDriverDetails);
	}

	@DeleteMapping("/driver/{id}")
	public ResponseEntity<Map<String, Boolean>> deleteDriver(@PathVariable Long id){
		DriverModel driverModel = driverRepository.findById(id)
			.orElseThrow(() -> new ResourceNotFoundException("DriverLocation not exist with id: " + id ));
		driverRepository.delete(driverModel);
